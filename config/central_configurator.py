...
def make_data_generator_configs(
    ...
    if save_name is not None:
        save_path = Path(save_folder) / save_name if save_folder else Path(save_name)
    ...
    return {
        ...
        "config_file_name": str(save_path) if save_name is not None else None,
    }


def get_data_generator_config(yaml_config_path: Path = None, base_path: Path = None):
...
def make_train_network_configs(
    ...
    if save_name is not None:
        ...
        lanfactory.utils.try_gen_folder(
            folder=save_folder, allow_abs_path_folder_generation=True
        )
...
def get_train_network_config(yaml_config_path=None, net_index=0):
    basic_config = yaml.safe_load(open(yaml_config_path, "rb"))
    ...
    config = make_train_network_configs(
...
def make_param_recovery_configs(
    model_name="ddm",
    ...
    save_folder = Path(save_folder) if save_folder else Path()
    save_file = save_folder / f"{model_name}_parameter_recovery_run_config.pickle"

    parameter_recovery_config_dict = {
        "parameter_recovery_data_loc": parameter_recovery_data_loc,
    ...
    with open(save_file, "wb") as f:
        pickle.dump(parameter_recovery_config_dict, f)
